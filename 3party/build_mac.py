...
def build_library(source_dir, arch):
    ...
    build_dir = "build/%s/%s" % (output, arch)
    bin_dir = "bin/%s" % arch
    cmake_mac.configure(source_dir=source_dir, bin_dir=bin_dir, build_dir=build_dir, arch=arch)
    cmake_mac.build(build_dir, "Release")
    cmake_mac.install(build_dir, "Release")
...
def build_apr_util(source_dir, arch):
    ...
    bin_dir = "bin/%s" % arch
    cmake_mac.configure(source_dir=source_dir, bin_dir=bin_dir, build_dir=build_dir, arch=arch,deps="-DAPR_INCLUDE_DIR=../apr/include -DAPR_LIBRARIES=../build/apr/%/Release")
    cmake_mac.build(build_dir, "Release")
...
if __name__ == "__main__":
    ...
    for platform in ["x64"]:
        for library_prefix in ["libexpat/expat","libssh2","libgit2"]:
            library = detect_library(library_prefix)
            build_library(library, platform)
            
    build_curl_library(detect_library("curl"),"x64")
