...
#include <QDebug>
namespace ady {
QMap<QString,int> COSParse::monthNameMap;




COSParse::COSParse(bool mlsd)
    :mlsd(mlsd)
{
...
bool COSParse::lineParse(QString& line,FileItem& entry)
{
    if(this->mlsd){
...
bool COSParse::parseAsUnix(QString &line, FileItem& entry)
{
...
bool COSParse::parseAsMlsd(QString &line, FileItem& entry)
{
...
bool COSParse::parseAsDos(QString &line, FileItem &entry)
{
    QStringList tokens = line.split(QRegExp("\\s+"));
    int size = tokens.size();
    if(size<4){
...
bool COSParse::parseShortDate(QString &date,QString &time,FileItem &entry)
{
    if(date.isEmpty()){
            return false;
    }
