...
{TPL}Panel::{TPL}Panel(long long id,QWidget* parent)
    :Panel(id,parent),
    ui(new Ui::{TPL}Panel)
{
    ui->setupUi(this);
    ...
    this->{tpl} = dynamic_cast<{TPL}*>(this->request());
    this->worker = new {TPL}Worker(id);
    this->worker->start();
    ...
    connect(ui->actionRefresh,&QAction::triggered,this,&{TPL}Panel::onActRefresh);
...
void {TPL}Panel::init(SiteRecord record)
{
    Panel::init(record);
    m_setting = new {TPL}Setting(record.data);
    m_dirSync = m_setting->dirSync();
    m_dirMapping = m_setting->dirMapping();

    this->{tpl}->setHost(this->site.host);
    this->{tpl}->setPort(this->site.port);
    this->{tpl}->setUsername(this->site.username);
    this->{tpl}->setPassword(this->site.password);
    if(!this->site.path.endsWith("/")){
        this->site.path += "/";
    }
    this->worker->append({TPL}Worker::W_LINK,0);
    this->worker->append({TPL}Worker::W_PASV,true);

}



NetworkRequest* {TPL}Panel::request()
{
    NetworkRequest* req = NetworkManager::getInstance()->request(this->getId());
    if(req==nullptr){
        req = NetworkManager::getInstance()->newRequest<{TPL}>(this->getId());
    }
    return req;
}

void {TPL}Panel::deleteFiles(QList<FileItem>items)
{
    ...
    data->nid = nid;
    this->worker->append({TPL}Worker::W_DELETE,(long long)data);
}
...
void {TPL}Panel::workTaskFinished(QString name,NetworkResponse* response)
{
    if(response==nullptr){
        return ;
    }
   ...
   // response->debug();
    ...
    if(name=={TPL}Worker::W_PASV){
        if(response->errorCode==0){
            this->worker->append({TPL}Worker::W_CHDIR,this->site.path);
        }else{
            //connect failed
            response->debug();
        }

    }else if(name=={TPL}Worker::W_CHDIR){
        //response->debug();
        ...
        if(response->errorCode==0){
            ...
            response->debug();
            MessageDialog::error(ui->treeView,tr("{TPL} request list folder failed,error :%1").arg(response->errorMsg));

        }

    }else if(name=={TPL}Worker::W_UNLINK){

    }else if(name=={TPL}Worker::W_DELETE){
    ...
    }else if(name=={TPL}Worker::W_CHMOD){
        int fileCount = response->params["fileCount"].toInt();
        long long nid = response->params["nid"].toLongLong();
        if(fileCount==0){
    ...
    }else if(name=={TPL}Worker::W_MKDIR){
        this->onActRefresh();
    }
    delete  response;
}
...
void {TPL}Panel::onItemActivated(const QModelIndex &index)
{
    int row = index.row();
    RemoteFileItemModel* model = static_cast<RemoteFileItemModel*>(ui->treeView->model());
    FileItem item = model->getItem(row);
    if(item.enabled && item.type==FileItem::Dir){
        this->worker->append({TPL}Worker::W_CHDIR,item.path);
    }
}

void {TPL}Panel::onDropUpload(const QMimeData* data)
{
...
void {TPL}Panel::onContextMenu(const QPoint& pos)
{
    Q_UNUSED(pos);
...
void {TPL}Panel::onActChmod()
{
    ...
    if(size>0){
        ...
        if(dialog->exec()==QDialog::Accepted){
            ...
            data->nid = nid;
            this->worker->append({TPL}Worker::W_CHMOD,(long long)data);
        }
    }
}

void {TPL}Panel::onActDelete()
{
...
void {TPL}Panel::onActRefresh()
{
    this->worker->append({TPL}Worker::W_CHDIR,this->currentDir);
}

void {TPL}Panel::onActRename(){
    QModelIndex index = ui->treeView->currentIndex();
    ui->treeView->edit(index);
}

void {TPL}Panel::onActMkdir(){
    NewFileDialog* dialog = new NewFileDialog(NewFileDialog::Folder,this->topLevelWidget());
    dialog->setPath(currentDir);
    int ret = dialog->exec();
    if(ret == QDialog::Accepted){
        QString filename = dialog->fileName();
        this->worker->append({TPL}Worker::W_MKDIR,filename);
...
void {TPL}Panel::onRename(const QModelIndex& index,QString newString)
{
