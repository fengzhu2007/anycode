...
#include <QDebug>
namespace ady {
QMap<QString,int> {TPL}Parse::monthNameMap;




{TPL}Parse::{TPL}Parse(bool mlsd)
    :mlsd(mlsd)
{
...
bool {TPL}Parse::lineParse(QString& line,FileItem& entry)
{
    if(this->mlsd){
...
bool {TPL}Parse::parseAsUnix(QString &line, FileItem& entry)
{
...
bool {TPL}Parse::parseAsMlsd(QString &line, FileItem& entry)
{
    QStringList tokens = line.split(";");
    int size = tokens.size();
    if(size>=3){
        entry.name = tokens.at(size - 1).mid(1);
    }else{
        return false;
    }
...
bool {TPL}Parse::parseAsDos(QString &line, FileItem &entry)
{
    QStringList tokens = line.split(QRegExp("\\s+"));
    int size = tokens.size();
    if(size<4){
...
bool {TPL}Parse::parseShortDate(QString &date,QString &time,FileItem &entry)
{
    if(date.isEmpty()){
            return false;
    }
