...
FTPPanel::FTPPanel(long long id,QWidget* parent)
    :Panel(id,parent),
    ui(new Ui::FTPPanel)
{
    ui->setupUi(this);
    ...
    this->ftp = dynamic_cast<FTP*>(this->request());
    this->worker = new FTPWorker(id);
    this->worker->start();

    //this->dirRender->setLocale(QLocale::Chinese);
    ...
    connect(ui->actionRefresh,&QAction::triggered,this,&FTPPanel::onActRefresh);
...
void FTPPanel::init(SiteRecord record)
{
    Panel::init(record);
    m_setting = new FTPSetting(record.data);
    m_dirSync = m_setting->dirSync();
    m_dirMapping = m_setting->dirMapping();

    this->ftp->setHost(this->site.host);
    this->ftp->setPort(this->site.port);
    this->ftp->setUsername(this->site.username);
    this->ftp->setPassword(this->site.password);
    if(!this->site.path.endsWith("/")){
        this->site.path += "/";
    }
    this->worker->append(FTPWorker::W_LINK,0);
}
...
void FTPPanel::deleteFiles(QList<FileItem>items)
{
    ...
    data->nid = nid;
    this->worker->append(FTPWorker::W_DELETE,(long long)data);
}
...
void FTPPanel::workTaskFinished(QString name,NetworkResponse* response)
{
    if(response==nullptr){
        return ;
    }
   ...
   // response->debug();
    ...
    if(name==FTPWorker::W_LINK){
        if(response->status()){
            this->worker->append(FTPWorker::W_PASV,true);
        }else{
            MessageDialog::error(ui->treeView,tr("FTP connect failed,error :%1").arg(response->errorMsg));
        }
    }else if(name==FTPWorker::W_PASV){
        if(response->errorCode==0){
            this->worker->append(FTPWorker::W_CHDIR,this->site.path);
        }else{
            //connect failed
            response->debug();
        }

    }else if(name==FTPWorker::W_CHDIR){
        //response->debug();

        if(response->errorCode==0){
            if(response->networkErrorCode==226){
                //response->debug();
                qDebug()<<"ok";
            ...
            //qDebug()<<"error code invalid:"<<response->errorCode;
            response->debug();
            MessageDialog::error(ui->treeView,tr("FTP request list folder failed,error :%1").arg(response->errorMsg));

        }

    }else if(name==FTPWorker::W_UNLINK){

    }else if(name==FTPWorker::W_DELETE){
    ...
    }else if(name==FTPWorker::W_CHMOD){
        int fileCount = response->params["fileCount"].toInt();
        long long nid = response->params["nid"].toLongLong();
        if(fileCount==0){
    ...
    }else if(name==FTPWorker::W_MKDIR){
        this->onActRefresh();
    }else if(name==FTPWorker::W_RENAME){
        if(response->status()){
            //refresh
            this->onActRefresh();
        }
...
void FTPPanel::onItemActivated(const QModelIndex &index)
{
    int row = index.row();
    RemoteFileItemModel* model = static_cast<RemoteFileItemModel*>(ui->treeView->model());
    FileItem item = model->getItem(row);
    if(item.enabled && item.type==FileItem::Dir){
        this->worker->append(FTPWorker::W_CHDIR,item.path);
    }
}

void FTPPanel::onDropUpload(const QMimeData* data)
{
...
void FTPPanel::onContextMenu(const QPoint& pos)
{
    Q_UNUSED(pos);
...
void FTPPanel::onActChmod()
{
    ...
    if(size>0){
        ...
        if(dialog->exec()==QDialog::Accepted){
            ...
            data->nid = nid;
            this->worker->append(FTPWorker::W_CHMOD,(long long)data);
        }
    }
}

void FTPPanel::onActDelete()
{
...
void FTPPanel::onActRefresh()
{
    this->worker->append(FTPWorker::W_CHDIR,this->currentDir);
}

void FTPPanel::onActRename(){
    QModelIndex index = ui->treeView->currentIndex();
    ui->treeView->edit(index);
}

void FTPPanel::onActMkdir(){
    NewFileDialog* dialog = new NewFileDialog(NewFileDialog::Folder,this->topLevelWidget());
    dialog->setPath(currentDir);
    int ret = dialog->exec();
    if(ret == QDialog::Accepted){
        QString filename = dialog->fileName();
        this->worker->append(FTPWorker::W_MKDIR,filename);
...
void FTPPanel::onRename(const QModelIndex& index,QString newString)
{
    QRegExp reg("^[^\?\\*|\"<>:/]{1,256}$");
    ...
    data.insert("dst",dst);
    this->worker->append(FTPWorker::W_RENAME,data);
