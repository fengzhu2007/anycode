...
#include <QDebug>
namespace ady {
QMap<QString,int> S3Parse::monthNameMap;




S3Parse::S3Parse(bool mlsd)
    :mlsd(mlsd)
{
...
bool S3Parse::lineParse(QString& line,FileItem& entry)
{
    if(this->mlsd){
...
bool S3Parse::parseAsUnix(QString &line, FileItem& entry)
{
...
bool S3Parse::parseAsMlsd(QString &line, FileItem& entry)
{
...
bool S3Parse::parseAsDos(QString &line, FileItem &entry)
{
    QStringList tokens = line.split(QRegExp("\\s+"));
    int size = tokens.size();
    if(size<4){
...
bool S3Parse::parseShortDate(QString &date,QString &time,FileItem &entry)
{
    if(date.isEmpty()){
            return false;
    }
