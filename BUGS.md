...
~~编辑器保存时 有时候资源管理器定位不正确,会定位到目录；由于保存时会触发多此目录变改变，资源管理器对应节点被刷新;~~  
~~编辑器在编写PHP代码时，.符号时连接符，不能唤起自动完成提示~~  
编辑器在编辑字符串内容时，也会触发自动完成的功能；需要取消自动完成的功能。
再jsx文件中当遇到类似<div>"{item.name}"</div>这样的字符时，token状态解析会有问题      
...
~~client区域先关闭其他文件，再通过资源管理器打开新文件时，tab的高亮状态不正确~~   
