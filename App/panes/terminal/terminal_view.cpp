...
void TerminalView::setFont(const QFont &font)
{
    ...
    d->m_cellSize = {qfm.averageCharWidth(), (double) std::ceil(qfm.height())};
...
static QPixmap generateWavyPixmap(qreal maxRadius, const QPen &pen)
{
    ...
    const int width = std::ceil(100 / (2 * halfPeriod)) * (2 * halfPeriod);
    const qreal radius = std::floor(radiusBase * 2) / 2.;
                                   ...
                                   const QRawFont &font)
{
    ...
    QLineF line(std::floor(pos.x()), pos.y(), std::floor(pos.x() + width), pos.y());
    ...
    if (underlineStyle == QTextCharFormat::WaveUnderline) {
        ...
        const QPixmap wave = generateWavyPixmap(qMin(qMax(underlineOffset, pen.widthF()),
                                                     maxHeight / qreal(2.)),
                                                pen);
        const int descent = std::floor(maxHeight);

        painter.setBrushOrigin(painter.brushOrigin().x(), 0);
        painter.fillRect(pos.x(), 0, std::ceil(width), qMin(wave.height(), descent), wave);
        painter.restore();
...
void TerminalView::paintCells(QPainter &p, QPaintEvent *event) const
{
    ...
    const int startRow = std::floor((qreal) event->rect().y() / d->m_cellSize.height()) + scrollOffset;
    const int endRow = qMin(maxRow,
                            (int)std::ceil((event->rect().y() + event->rect().height())
                                  / d->m_cellSize.height())
                                + scrollOffset);
